...
class GoogleMyBusinessService
{
     ...
     *
     * @param ConnectedPlatform $platform
     * @return array Öffnungszeiten im Frontend-Format
     */
    public function getBusinessHours(ConnectedPlatform $platform): array
    {
        ...
        // Locations List API verwenden (locations/XXX kann nicht direkt abgerufen werden)
        $response = Http::withToken($token)
        ...
        if (!$response->successful()) {
            Log::error('Google Öffnungszeiten abrufen fehlgeschlagen', [
                'platform_id' => $platform->id,
                'status' => $response->status(),
                'body' => $response->body(),
            ]);

            throw new \Exception('Öffnungszeiten konnten nicht abgerufen werden.');
        }

        $data = $response->json();
        $locations = $data['locations'] ?? [];

        // Finde unsere Location in der Liste
        $location = collect($locations)->firstWhere('name', $locationName);

        if (!$location) {
            throw new \Exception('Location nicht in der Liste gefunden.');
        }

        $periods = $location['regularHours']['periods'] ?? [];

        // Konvertiere Google Format zu unserem Format
        return $this->convertGoogleHoursToOurFormat($periods);
    }

    /**
     * Aktualisiert die Öffnungszeiten einer Location in Google
     *
     * PATCH https://mybusinessbusinessinformation.googleapis.com/v1/{locationName}
     * Mit updateMask: regularHours
     *
     * @param ConnectedPlatform $platform
     * @param array $hours Öffnungszeiten im unserem Format
     * @return bool true wenn erfolgreich
     */
    public function updateBusinessHours(ConnectedPlatform $platform, array $hours): bool
    {
        $token = $this->getAccessToken($platform);
        $locationName = $platform->metadata['location_name'] ?? null;
        $accountName = $platform->metadata['account_name'] ?? null;
        ...
        // Konvertiere unser Format zu Google Format
        $googleHours = $this->convertOurHoursToGoogleFormat($hours);

        $response = Http::withToken($token)
            ->patch(self::BUSINESS_INFO_API . "/{$fullLocationPath}?updateMask=regularHours", [
                'regularHours' => [
                    'periods' => $googleHours,
                ],
            ]);
        ...
        if (!$response->successful()) {
            Log::error('Google Öffnungszeiten aktualisieren fehlgeschlagen', [
                'platform_id' => $platform->id,
                'status' => $response->status(),
                'body' => $response->body(),
            ]);

            throw new \Exception('Öffnungszeiten konnten nicht aktualisiert werden: ' . $response->json('error.message', 'Unbekannter Fehler'));
        }

        Log::info('Google Öffnungszeiten erfolgreich aktualisiert', [
            'platform_id' => $platform->id,
        ]);
    ...
    /**
     * Konvertiert Google Öffnungszeiten Format zu unserem Format
     *
     * Google: [{ openDay: "MONDAY", openTime: "09:00", closeDay: "MONDAY", closeTime: "18:00" }]
     * Unser: { monday: { open: "09:00", close: "18:00", closed: false } }
     */
    private function convertGoogleHoursToOurFormat(array $periods): array
    {
        ...
        // Öffnungszeiten aus Google einsetzen
        foreach ($periods as $period) {
            $googleDay = $period['openDay'] ?? null;
            $ourDay = $dayMap[$googleDay] ?? null;

            if ($ourDay) {
                // openTime kann ein leeres Array sein (= 00:00) oder ein Objekt mit hours/minutes
                $openTime = $this->parseGoogleTime($period['openTime'] ?? []);
                // closeTime kann auch ein Objekt sein, z.B. {"hours": 24} für Mitternacht
                $closeTime = $this->parseGoogleTime($period['closeTime'] ?? []);
    ...
    /**
     * Parst Google Zeit-Format zu HH:MM
     *
     * Google Format kann sein:
     * - [] (leeres Array = 00:00)
     * - {"hours": 9, "minutes": 30} (= 09:30)
     * - {"hours": 24} (= 23:59, Mitternacht)
     */
    private function parseGoogleTime($time): string
    {
        ...
        // Spezialfall: 24:00 = 23:59 (Mitternacht)
        if ($hours >= 24) {
            return '23:59';
        }
    ...
    /**
     * Konvertiert unser Öffnungszeiten Format zu Google Format
     *
     * Unser: { monday: { open: "09:00", close: "18:00", closed: false } }
     * Google: [{ openDay: "MONDAY", openTime: {...}, closeDay: "MONDAY", closeTime: {...} }]
     */
    private function convertOurHoursToGoogleFormat(array $hours): array
    {
        $dayMap = [
            'monday' => 'MONDAY',
            'tuesday' => 'TUESDAY',
        ...
        foreach ($hours as $day => $schedule) {
            $googleDay = $dayMap[$day] ?? null;

            // Nur Tage hinzufügen, die NICHT geschlossen sind
            if ($googleDay && !($schedule['closed'] ?? true)) {
                $openTime = $this->formatTimeForGoogle($schedule['open'] ?? '00:00');
                $closeTime = $this->formatTimeForGoogle($schedule['close'] ?? '23:59');
    ...
    /**
     * Formatiert Zeit von HH:MM zu Google Format
     *
     * "00:00" → [] (leeres Array)
     * "09:30" → {"hours": 9, "minutes": 30}
     * "23:59" → {"hours": 24} (Mitternacht)
     */
    private function formatTimeForGoogle(string $time): array
    {
        ...
        // Spezialfall: 00:00 = leeres Array
        if ($hours === 0 && $minutes === 0) {
            return [];
        }

        // Spezialfall: 23:59 = 24:00 (Mitternacht)
